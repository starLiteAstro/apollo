...
class Vote(Base):
    __tablename__ = "vote"
    id: Mapped[int_pk] = mapped_column(primary_key=True, init=False)
...
class UserVote(Base):
    ...
    vote_choice: Mapped[VoteChoice] = relationship(back_populates="user_votes", init=False)
...
class DiscordVoteMessage(Base):
    ...
    discord_vote: Mapped["DiscordVote"] = relationship(back_populates="messages", init=False)
...
class DiscordVote(Base):
    ...
    messages: Mapped["DiscordVoteMessage"] = mapped_column(back_populates="discord_vote", cascade="all, delete-orphan")
